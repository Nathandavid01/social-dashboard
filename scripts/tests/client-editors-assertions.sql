...
do $$begin
 perform set_recording_client_editors(uid(20),uid(10),array[uid(2),uid(3)]);
 if (select count(*) from client_editor_assignments where client_id=uid(10))<>2 then raise exception 'missing editor links';end if;
 if (select client_id from recording_sessions where id=uid(20)) is distinct from uid(10) then raise exception 'session not linked';end if;
 if (select assigned_to from clients where id=uid(10)) is distinct from uid(2) then raise exception 'primary changed';end if;
 begin
  perform set_recording_client_editors(uid(20),uid(10),array[uid(4)]);
 ...
 perform set_recording_client_editors(uid(20),uid(10),array[uid(3)]);
...
do $$begin
 begin
  perform set_recording_client_editors(uid(20),uid(10),array[uid(2)]);
...
begin;
select set_config('test.actor',uid(1)::text,true);
select set_recording_client_editors(uid(20),uid(10),array[uid(2),uid(3)]);
