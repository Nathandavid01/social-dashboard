...
- Aníbal Fuentes PNP está en pausa, así que sus cortes no entran aunque Eric los haya subido.
...
- Un video de un cliente que no está en modo AI no entra.
- El basurero pide confirmación antes de quitar el video de Recibo. No se programa en Metricool.
...
- Aplica al botón **+** de las tarjetas, a **Subir crudo** de On Site, al
  Banco y a **Enviar videos editados** del Pipeline. Cambiar el nombre del
  archivo no engaña a la huella; un re-export distinto sí es otro video.
...
![Video repetido detectado](/changelog/v5.96-videos-sin-duplicados.png)
...
**Toast arriba a la derecha cuando el cliente aprueba o pide cambios en /aprobacion.**
- Al votar en el enlace público, el staff recibe aviso en la campana y un toast (si tiene la preferencia activa).
- Toggle en **Cuenta → Notificaciones**: «Avisos emergentes de revisión del cliente» (activo por defecto).
- Apagar el toast **no** cambia el guardado de votos en `entregas_client_review`. No publica solo a Metricool.
...
- Mientras sube (o la IA lee el archivo), **Detener** corta el PUT y deja la página vacía.
- Caption, verificación y publicar usan solo el archivo de esta subida. El GFX que ya estaba no se trabaja.

## v4.90 — 2026-09-14

**Primer Round: el reproductor muestra el archivo que acabas de elegir.**
- Ya no se queda el promo anterior si la página se recarga a mitad de la subida.
...
- El caption lleva **@rafaellenin** y **@denniseyperez**.

## v4.86 — 2026-09-13

**Primer Round: la frase de Facebook.**
- El caption usa lo que ellos ponen siempre: **Mañana desde las 5:43 AM junto a Rafael Lenín López y Dennise Pérez.**

## v4.85 — 2026-09-13

**Primer Round: el gancho no se repite.**
- Si el caption dice `LA NOTICIA NO ESPERA` y la línea siguiente vuelve a empezar igual, se deja solo el horario: **mañana a las 5:43am en Primer Round…**
...
![Selección de editores (simulación)](/changelog/v4.67-editores-cliente.png)
...
- "Verificación" desaparece del menú: ahora es un tab dentro de **Publicados**,
  al lado de Por Fecha, Calendario y Por Cliente.
- El link viejo /schedule-check redirige solo — nada se rompe.
- El tab de Verificación solo lo ve quien tenía acceso a esa pantalla.

## v4.1 — 2026-08-31

**Menú más corto: Runway sale de la barra lateral.**

- El runway por cliente ya se ve dentro de Pipeline, así que la entrada
  "Runway" desaparece del menú de la izquierda.
- La página completa (plan semanal + tablero) sigue viva: se entra desde la
  tarjeta de Runway en Inicio.
...
- El botón "Buscar..." que estaba en la barra superior se removió. La paleta de comandos sigue disponible: usa **⌘P** o **⌘/** (Mac), o **Ctrl+K** (navegador) para abrirla.
- Un cambio mínimo de interfaz, sin impacto en flujos.
...
- La tira de 5 escenas debajo de un video (para videos viejos, sin thumbnails guardados) ahora sí reparte las imágenes por todo el video — antes, por un error de cálculo, las 5 salían apretadas dentro del primer segundo, así que la tira mostraba el arranque una y otra vez en vez de escenas del video completo.

Vista: [preview HTML](/previews/v3.38-caption-sin-hook.html)
...
- El portal **avisa antes del clic**: "al aprobar, el video queda programado para publicarse y la aprobación ya no se puede deshacer".
...
- **Borrar una tarea de Producción ahora pide confirmación bonita.** La tarjeta de tarea en **Producción** usaba el cuadro gris del navegador (`confirm()`) para eliminar; ahora abre el diálogo de confirmación en español con botón rojo, bloquea el cierre mientras borra y **avisa con un toast** ("Tarea eliminada" o un error claro). Mismo patrón que el resto de la app.

## v2.77 — 2026-06-21

- **Calendario en español.** El cuadro para **programar contenido** (Calendario → agregar evento) estaba en inglés; ahora todo está en español: título, plataforma, **estado** (Borrador/Programado/Publicado), cliente, fecha y los botones (Cancelar/Programar), incluyendo los mensajes de validación y el aviso de "Evento creado".
