...
it('does not retry creation automatically after an uncertain result', async () => {
 crearEnlaceCliente.mockRejectedValueOnce(Error('network'))
 setup()
 fireEvent.click(await screen.findByRole('button',{name:/generar enlace de aprobación/i}))
 expect(await screen.findByRole('alert')).toBeInTheDocument()
 expect(crearEnlaceCliente).toHaveBeenCalledTimes(1)
 getEnlaceCliente.mockResolvedValueOnce({enlace:ENLACE})
 fireEvent.click(screen.getByRole('button',{name:'Reintentar Enlace'}))
 expect(await screen.findByRole('link',{name:/abrir/i})).toBeInTheDocument()
 expect(crearEnlaceCliente).toHaveBeenCalledTimes(1)
})
it('does not claim successful creation when the response lacks a token', async()=>{
 crearEnlaceCliente.mockResolvedValueOnce({})
 setup()
 fireEvent.click(await screen.findByRole('button',{name:/generar enlace de aprobación/i}))
 expect(await screen.findByRole('alert')).toBeInTheDocument()
 expect(toast).not.toHaveBeenCalledWith(expect.objectContaining({title:'Enlace de Acme'}))
})
it('shows a copy failure without claiming the link was copied',async()=>{
 getEnlaceCliente.mockResolvedValue({enlace:ENLACE})
 vi.mocked(navigator.clipboard.writeText).mockRejectedValueOnce(Error('denied'))
 setup()
 fireEvent.click(await screen.findByRole('button',{name:/copiar enlace/i}))
 await waitFor(()=>expect(toast).toHaveBeenCalledWith(expect.objectContaining({title:'No Se Pudo Copiar',variant:'destructive'})))
 expect(toast).not.toHaveBeenCalledWith(expect.objectContaining({title:'Enlace copiado'}))
})
it('releases the download button after a rejected request',async()=>{
 getEnlaceCliente.mockResolvedValue({enlace:ENLACE})
 getEntregaVideoEditado.mockResolvedValue({id:'v1'})
 getEntregasDownloadUrl.mockRejectedValueOnce(Error('network'))
 setup()
 const button=await screen.findByRole('button',{name:/bajar el video/i})
 fireEvent.click(button)
 await waitFor(()=>expect(toast).toHaveBeenCalledWith(expect.objectContaining({title:'No se pudo descargar',variant:'destructive'})))
 expect(button).toBeEnabled()
 expect(window.open).not.toHaveBeenCalled()
})

it('does not announce readiness until the created token can be read back',async()=>{
 crearEnlaceCliente.mockResolvedValueOnce({token:'new'})
 getEnlaceCliente.mockResolvedValue({enlace:null})
 setup()
 fireEvent.click(await screen.findByRole('button',{name:/generar enlace de aprobación/i}))
 expect(await screen.findByRole('alert')).toBeInTheDocument()
