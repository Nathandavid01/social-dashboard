  ...
  return (
    <article id={`pipeline-idea-${clip.ideaId}`} data-testid={testId} data-pipeline-focus={focused ? 'true' : undefined} className={cn('group min-w-0 [&:has(details[open])]:col-span-2 overflow-hidden rounded-lg border bg-[#0d1013]', focused ? 'border-[#c8a34a] ring-2 ring-[#c8a34a]/50' : 'border-white/10')} style={{ borderColor: focused ? undefined : `${client.cardColor}66` }}>
      ...
      <div className="min-w-0 p-2">
        ...
        <p className="mt-0.5 text-[9px] font-medium uppercase tracking-[0.12em] text-slate-500">Idea</p>
        <p className="line-clamp-2 text-[10px] leading-snug text-slate-300">{clip.title}</p>
        {clip.linkKind && (
          <span className={`mt-1 inline-flex rounded-full border px-1.5 py-0.5 text-[8px] font-semibold uppercase ${clip.linkKind === 'linked' ? 'border-emerald-400/30 bg-emerald-400/10 text-emerald-300' : 'border-white/15 bg-white/5 text-slate-400'}`}>
            {clip.linkKind === 'linked' ? 'De la idea' : 'Extra'}
          </span>
        )}
