...
import {BANK_KIND_LABELS, type ClientBankFile} from '@/lib/utils/client-asset-bank'

export function EditorWorkPackage({clip,broll=[],bank=[]}:{clip:EditorBankClip;broll?:GlobalBrollFile[];bank?:ClientBankFile[]}){
 ...
 async function downloadBank(asset:ClientBankFile){
  setMessage('Preparando Descarga…')
  try{
   const result=await getClientAssetDownloadUrl(asset.id)
   if(!result.url)throw Error(result.error||'Archivo No Disponible')
   const a=document.createElement('a');a.href=result.url;a.target='_blank';a.rel='noopener noreferrer';a.download=asset.name;document.body.appendChild(a);a.click();a.remove();setMessage('Descarga Solicitada · '+asset.name)
  }catch(e){setMessage(e instanceof Error?e.message:'No Se Pudo Descargar')}
 }
 async function submit(){if(!file||!captions)return;setBusy(true);try{
  ...
  // Upload+register+submit is the only trigger: status becomes producida/submitted
  // without a separate "marcar hecho" step.
  setSent(true);setMessage(result.warning||'Editado · Hecho · Enviado A Revisión');router.refresh()
 }catch(e){setMessage(e instanceof Error?e.message:'No Se Pudo Confirmar La Entrega')}finally{setBusy(false)}}
 return <details className="mt-2 rounded-lg border border-sky-500/30 bg-sky-500/5 p-2 text-xs"><summary className="min-h-9 cursor-pointer py-2 font-semibold text-sky-300">Abrir Material Y Entregar</summary><div className="space-y-3 pt-2"><section className="space-y-1"><h4 className="font-semibold">1 · Idea E Instrucciones</h4>{[clip.hook,clip.visualBrief,clip.shootingNotes].filter(Boolean).map((text,i)=><p key={i} className="whitespace-pre-wrap break-words text-slate-300">{text}</p>)}{!clip.hook&&!clip.visualBrief&&!clip.shootingNotes&&<p className="text-slate-400">Esta Idea Todavía No Tiene Instrucciones.</p>}</section><section><h4 className="mb-2 font-semibold">2 · Descargar Material · {files.length} Archivos</h4><ul className="space-y-2">{files.map(source=><li key={source.id} className="rounded border border-white/10 p-2"><span className="block break-words">{source.name}</span><button type="button" onClick={()=>download(source)} className="mt-1 min-h-9 text-sky-300 underline">Descargar {source.kind==='broll'?'B-roll':'Crudo'}</button></li>)}</ul></section><section data-testid="client-asset-bank"><h4 className="mb-2 font-semibold">3 · Banco Del Cliente</h4>{bank.length===0?<p className="text-slate-400">Este cliente aún no tiene archivos en su banco.</p>:<ul className="space-y-2">{bank.map(asset=><li key={asset.id} className="rounded border border-white/10 p-2"><span className="block break-words">{asset.name}</span><p className="text-[10px] text-slate-400">{BANK_KIND_LABELS[asset.kind]}</p><button type="button" onClick={()=>void downloadBank(asset)} className="mt-1 min-h-9 text-sky-300 underline">Descargar {BANK_KIND_LABELS[asset.kind]}</button></li>)}</ul>}</section>{canUpload&&<section className="space-y-2"><h4 className="font-semibold">4 · Entregar El Editado</h4><p className="text-slate-400">Al subir y enviar, la pieza pasa a Editado/Hecho sola — no hay que marcarla a mano. Si el supervisor pide cambios, recibirás el comentario para corregir y reenviar.</p><input aria-label="Video Editado" type="file" accept="video/*" disabled={busy||sent} onChange={e=>{setFile(e.target.files?.[0]??null);setRegistered(null);setCaptions(false)}} className="block w-full min-w-0 text-xs"/><label className="flex items-start gap-2"><input type="checkbox" checked={captions} disabled={busy||sent} onChange={e=>setCaptions(e.target.checked)}/>El Editado Incluye Subtítulos Y Revisé Audio, Marca Y Cierre</label><button type="button" disabled={!file||!captions||busy||sent} onClick={submit} className="min-h-11 rounded-lg bg-sky-600 px-3 font-semibold text-white disabled:opacity-50">{busy?'Enviando…':sent?'Editado · Hecho':'Enviar A Revisión'}</button>{sent&&<span data-testid="editado-hecho-chip" className="inline-flex items-center rounded-md border border-emerald-500/40 bg-emerald-500/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-emerald-300">Editado · Hecho</span>}</section>}{message&&<p role="status" className="break-words text-sky-200">{message}</p>}</div></details>
}
