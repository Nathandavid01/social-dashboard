...
export async function AssignedRecordings({ memberId }: { memberId?: string }) {
  ...
  return <section className="mb-6 rounded-xl border border-sky-500/30 bg-sky-500/5 p-4 sm:p-5">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
      ...
      <Link className="text-sm text-sky-600 dark:text-sky-400 underline" href={calendarHref}>
        {overview ? 'Ver calendario' : 'Ver Mi Calendario'}
      </Link>
    </div>
    <p className="mb-3 text-xs text-muted-foreground">
      {overview
        ? 'Todas las próximas · Hora de Puerto Rico · Confirmada / Sin confirmar por sesión.'
        : 'Próximas 30 sesiones · Hora de Puerto Rico · Consulta aquí tu próxima grabación.'}
    </p>
    {result.error ? <p role="alert">{result.error}</p> : !result.sessions.length ? (
