...
- [ ] **`0062_video_thumb_keys.sql`** — columna `content_idea_videos.thumb_keys` (tira de 5 escenas). Degrada seguro: sin la columna, `registerVideoThumbs` devuelve `{error}` sin lanzar y `getVideoThumbViewUrls` devuelve `{ urls: [] }`; el panel cae siempre al fallback al vuelo (canvas sobre el preview firmado).
- [ ] **`0063_analysis_frame_count.sql`** — columna `content_idea_video_analysis.frame_count` (cuántos fotogramas analizó la IA, v3.39). Degrada seguro: la ruta escribe `frame_count` con un `UPDATE` separado y best-effort — si la columna no existe, ese `UPDATE` simplemente no hace nada (status/findings del análisis no se ven afectados), y `qc-progress-dots.tsx` no muestra la línea del contador cuando `frameCount` es null.
- [ ] **`0064_hook_source.sql`** — columna `content_ideas.hook_source` (marca "escrito por la IA a partir del video", v3.40). Degrada seguro: el hook se sigue escribiendo igual desde `video_topic` (UPDATE separado, best-effort) y la UI simplemente no muestra la marca hasta aplicarla.
- [ ] **`0072_video_fingerprints.sql`** — tabla `content_idea_video_fingerprints` (huella por video subido, v4.10: nunca se sube un video repetido). Degrada seguro: sin la tabla, `findDuplicateVideo` devuelve null (la subida sigue) y `rememberVideoFingerprint` se traga el error. Sin aplicarla NO hay deduplicación.
- [ ] **`0045_daily_video_capacity.sql`** — columna `profiles.daily_video_capacity`. Verificado ausente en prod el 2026-09-01: el diálogo "Editar nombre y título" de /settings/users falla al guardar capacidad.
- [ ] **`0067_user_time_days.sql`** — tabla `user_time_days` (Jornada: segundos activos por persona y día). Sin ella, el latido responde "Jornada no disponible" y `/actividad` muestra el tablero vacío. Degrada seguro.
...
- [x] **`0041_caption_feedback.sql`** — aplicada en prod (verificado 2026-08-15: tabla con filas).
- [x] **`0033_idea_lab_feedback.sql`** — aplicada en prod (verificado 2026-08-15: tabla con filas).
- [x] **`0054`/`0055`/`0057`/`0058`/`0059`** — aplicadas en prod (review de Entregas, caption_draft, drop FK, ui_events verificadas 2026-08-15).
...
- [~] **Approve/reject + learning loop (#4)** — ✓/✗ on each Lab idea (`rateIdea`); approved ideas show in **Ideas Aprobadas** (`/ideas-aprobadas`, gated `ideas.read`) for editors/designers; both verdicts feed back into generation (`getIdeaFeedbackForPrompt` → `formatFeedbackForPrompt`). Migration **`0033_idea_lab_feedback.sql`** aplicada en prod (verificado 2026-08-15) — persiste end-to-end. Later: also pull the *posted* idea's real Metricool performance back in (close the full loop).
