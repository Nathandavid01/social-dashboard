...
- [ ] **`0089_video_fingerprints.sql`** — tabla `content_idea_video_fingerprints` (huella por video subido, v5.96: nunca se sube un video repetido). Degrada seguro: sin la tabla, `findDuplicateVideo` devuelve null (la subida sigue) y `rememberVideoFingerprint` se traga el error. Sin aplicarla NO hay deduplicación.
