...
## Filtro I (`/filtro-i`) y Grok-ing (`/grok-ing`)
- [~] **Revisión automática del video entregado.** El editor sube el video en Filtro I → se sacan 24 frames en el navegador (`<video>`+`<canvas>`) → WhisperAPI transcribe el audio con timestamps → `grok-4.5` lee frames + transcripción y devuelve tabla de errores + Caption Base → el Caption Base pasa por el agente de captions existente (`buildIdeaCaptionPrompt`, con historial de Metricool + aprobados + rechazados). El editor ve **solo la tabla de errores**; el caption vive en **Grok-ing** (`grok_ing.read`, sin acceso para editor ni diseñador). **BLOQUEADO**: aplicar `0056_filtro_i_analisis.sql` en Supabase. Hasta entonces entregar videos funciona y la lista de análisis sale vacía (degrada, no rompe).
- [ ] **Clave de WhisperAPI válida.** La actual da 401 (`invalid API key ... or unused credits`) contra `transcribe.whisperapi.com`. Regenerar en lemonfox.ai/apis/keys y poner `WHISPERAPI_API_KEY` en `.env.local` y en Vercel.
- [ ] **Confirmar el modelo de visión.** `GROK_VISION_MODEL` por defecto `grok-4.5`; verificar que la key de xAI lo tiene habilitado.
- [ ] **Integrar Grok-ing con Copy.** Hoy Grok-ing solo enseña y deja copiar el caption. `caption_final` ya vive en la misma fila que el resto del análisis para que enganchar con la etapa Copy de /entregas sea barato.
- [ ] **Aprobar desde Filtro I.** El flujo pedía que el equipo "verifique y apruebe"; hoy la tabla es informativa y el editor corrige y vuelve a subir. Decidir si hace falta un estado de aprobación propio o si el de /revision basta.
- [ ] **Safari.** La extracción usa `<video>`+`<canvas>`; en Safari algunos mp4/AAC fallan al decodificar. Hoy avisa con un mensaje claro y no sube el video; probar con material real y decidir si hace falta un camino alterno.
