...
### Staged 0077 — public review file identity

After pushing 45558a1, continued the exact-file audit. Current public RPC/caller uses idea identity alone and resolves latest media; this does not establish which cut the client voted on. Added separate v2 read/vote RPCs in 0077, leaving active callers unchanged pending coordinated migration and UI rollout. The vote checks the displayed edited Entregas R2 file against current available media, seals its ID, and preserves atomic rejection/activity and sent-video protection. Readback of a decided vote uses the sealed file, never a newer replacement; legacy unsealed votes expose no media.

Test-first local PostgreSQL run failed on missing v2 RPC. After implementation, assertions passed for stale replacement, failed/missing media, persisted approved file, anonymous token/idea boundaries, editor correction return/activity, sent-video rejection guard and legacy unsealed votes. Fixture is minimal, not full production schema/RLS. No Supabase migration, live client vote or Metricool action occurred. Follow-up requirements are in scripts/tests/README-public-vote-file.md. No UI version increment because these functions remain staged and unused.

### v4.62 — monthly recording preparation tasks

Added the requested orange Grabación sidebar badge and expandable actionable checklist. Counts each current Puerto Rico month session once when client, videographer, editor, posting-derived target, or sufficient assigned non-discarded titled ideas are missing; excludes completed/cancelled sessions. Counts only ideas belonging to both the session and client. Queries retain caller RLS and recording.read, paginate completely, and surface errors rather than false zeroes. Refresh on route/focus, preparation state updates and 30-second visible-page interval. Checklist opens existing session details; unknown-to-current-calendar sessions route to On Site.

Verified authenticated localhost:3038: badge 31, checklist 31, and calendar summary 31 sessions without videographer. Inspected expanded checklist screenshot. Mobile standalone preview is explicitly simulated. 37 focal tests pass; TypeScript and merge-gate pass. No production data edited and no migrations needed for this feature.

### v4.63 — assignments per person in Profile and Mi Día

Added a shared personal/team task surface using the existing tasks table. Managers with tasks.create/read.all can select an active approved person, instructions, deadline (17:00 Puerto Rico) and priority. Regular users read only effective assignee tasks; status writes additionally filter by assignee unless authorized for team edits. Explicit row acknowledgement prevents zero-row success. UUID request identity reconciles matching duplicate insert retries; transport uncertainty preserves the submitted fields. Notification insertion is best effort with a separate saved-task warning. Existing team boards and operational task storage remain shared.

Verified authenticated /account/profile renders Eric Owner, active people options and assignment form. No fabricated task or notification sent to real teammates for QA. 11 focal tests pass (permissions, failed reads, target validity, notification warning, duplicate reconciliation, own-task update boundaries, form/role UI). Full suite: 395 files passed, 1 skipped; 3115 tests passed, 3 skipped. TypeScript and merge-gate pass. Mobile standalone preview inspected, explicitly simulated; actual production write/notification delivery and other-role live browser verification remain unverified. No new migration required. Version 4.63.

### v4.64 — supervisors always access editing
