...
export default async function AuthLayout({ children }: { children: React.ReactNode }) {
  const [realReach, brandingRaw] = await Promise.all([
    reachWithTimeout(),
    getAgencyBranding(),
  ])
  // Login always shows a brand mark; use custom agency branding when enabled.
  const branding = brandingRaw.apply_on_login ? brandingRaw : DEFAULT_AGENCY_BRANDING

  return (
    <div className="min-h-screen w-full bg-background lg:grid lg:grid-cols-[1.05fr_1fr] xl:grid-cols-[1.15fr_1fr]">
      {/* Brand showcase — always dark (negro + dorado) */}
      <aside className="relative hidden overflow-hidden bg-zinc-950 p-10 text-zinc-100 lg:flex lg:flex-col lg:justify-center lg:gap-12 xl:p-14">
        <div aria-hidden className="pointer-events-none absolute inset-0 motion-reduce:[&_*]:!animate-none">
          <div
            className="absolute -left-32 -top-32 h-96 w-96 rounded-full blur-3xl motion-safe:animate-aurora1"
        ...
        <div className="relative motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 duration-500">
          <AgencyMark branding={branding} size={54} showWordmark onDark />
        </div>
        ...
        <div className="relative max-w-md">
          ...
          <ul className="mt-10 space-y-5">
            {FEATURES.map((f, i) => (
              ...
              >
                <span
                  className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-lg border text-primary"
                  style={{
        ...
        <p className="relative text-xs text-zinc-500 lg:absolute lg:inset-x-10 lg:bottom-8 xl:inset-x-14">
          © 2026 {branding.brand_name} · Hecho para agencias que se toman en serio su contenido.
        </p>
      </aside>
      ...
      <main className="flex min-h-screen items-center justify-center px-6 py-12 sm:px-10">
        <div className="w-full max-w-sm">
          <div className="mb-8 flex flex-col items-center text-center lg:hidden">
            <AgencyMark branding={branding} size={56} showWordmark className="flex-col items-center" wordmarkClassName="items-center text-center" />
          </div>
          {children}
        </div>
      </main>
