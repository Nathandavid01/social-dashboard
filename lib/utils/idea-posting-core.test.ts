...
describe('ideaPostReadiness', () => {
  it('is ready when approved + caption + edited video + not posted + not published', () => {
    expect(ideaPostReadiness(ready, true)).toEqual({ ready: true })
  })

  it('never re-posts once metricool_post_id is set (idempotency, checked first)', () => {
    const r = ideaPostReadiness({ ...ready, metricool_post_id: 12345 }, true)
    expect(r.ready).toBe(false)
    expect(r.reason).toMatch(/ya se publicó/i)
  })

  it('blocks when not approved', () => {
    expect(ideaPostReadiness({ ...ready, approval_status: 'submitted' }, true).ready).toBe(false)
  })

  it('blocks when the caption is missing or blank', () => {
    expect(ideaPostReadiness({ ...ready, generated_caption: null }, true).ready).toBe(false)
    expect(ideaPostReadiness({ ...ready, generated_caption: '   ' }, true).ready).toBe(false)
  })

  it('blocks when there is no edited video', () => {
    expect(ideaPostReadiness(ready, false).reason).toMatch(/video editado/i)
  })

  it('blocks when already published', () => {
    expect(ideaPostReadiness({ ...ready, published_at: '2026-06-01' }, true).ready).toBe(false)
    expect(ideaPostReadiness({ ...ready, status: 'publicada' }, true).ready).toBe(false)
  })
})

describe('buildPublishDateTime', () => {
  it('schedules at the planned publish_date + the client posting_time', () => {
    expect(buildPublishDateTime('2026-06-15', '14:30')).toBe('2026-06-15T14:30:00')
  })

  it('defaults to 10:00 when the client has no posting_time', () => {
    expect(buildPublishDateTime('2026-06-15', null)).toBe('2026-06-15T10:00:00')
  })
