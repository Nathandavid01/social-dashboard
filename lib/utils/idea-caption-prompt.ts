...
export interface IdeaCaptionPromptInput {
  ...
  /** LIVE clip vs GFX/promo for Primer Round. Default GFX (existing studio). */
  primerRoundPieceKind?: PrimerRoundPieceKind
}
...
export function buildIdeaCaptionPrompt(input: IdeaCaptionPromptInput): string {
  if (input.primerRoundLockedTemplate) {
    return buildPrimerRoundCaptionPromptInstructions({
      ...
      pieceKind: input.primerRoundPieceKind,
...
- Nunca inventes diálogo ni palabras dichas. Si no hay transcripción, no cites frases como si se hubieran oído
