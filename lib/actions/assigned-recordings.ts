...
export async function getAssignedRecordings(memberId?: string) {
  ...
  if (memberId && memberId !== effectiveId && memberId !== user.id && !await currentUserHas('team.read')) return null
  ...
  return { memberId: memberId || effectiveId, sessions: (data ?? []) as RecordingSession[], error: error ? 'No se pudo cargar la agenda asignada.' : null }
}
