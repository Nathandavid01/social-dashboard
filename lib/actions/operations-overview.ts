...
export async function getOperationsOverview():Promise<{data?:OperationsOverview;error?:string}|null>{
 ...
 const [ideas,clients,editors,activity]=await Promise.all([
  ...
  db.from('clients').select('id,name,assigned_to,posting_days,posting_time,posting_schedule,metricool_blog_id', { count: 'exact' }).eq('status','active').order('id').limit(5000),
