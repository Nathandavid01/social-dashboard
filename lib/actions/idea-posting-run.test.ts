...
function database() {
 return {from(table:string) {
  let write=false
  const q:any={
    select:()=>q,eq:()=>q,is:()=>q,or:()=>q,in:()=>q,neq:()=>q,
    update:(payload:Record<string,unknown>)=>{write=true;h.writes.push(payload);return q},
    single:async()=>({data:h.idea,error:null}),
    then:(resolve:any)=>resolve({data:write?[{id:'idea'}]:table==='content_idea_videos'?[{id:'approved-file',idea_id:'idea',kind:'edited',status:'uploaded',storage_provider:'entregas-r2',drive_file_id:'edited.mp4'}]:[],error:null}),
  };return q
 }} as any
}
beforeEach(()=>{
 vi.useFakeTimers();vi.setSystemTime(new Date('2026-09-07T14:00:00Z'))
 h.writes=[];h.post.mockReset().mockResolvedValue({data:{id:123,uuid:'remote'}});h.health.mockReset().mockResolvedValue({ok:true})
...
describe('approval to Metricool boundary',()=>{
 it('schedules the exact approved Entregas video as a real post on tomorrow\'s date',async()=>{
  expect(await runIdeaPost(database(),'idea','owner')).toMatchObject({ok:true})
 ...
 it.each([null,'2026-08-01'])('blocks missing/stale dates before claiming or posting: %s',async date=>{
  h.idea.publish_date=date;expect(await runIdeaPost(database(),'idea','owner')).toHaveProperty('skipped');expect(h.writes).toHaveLength(0);expect(h.post).not.toHaveBeenCalled()
 })
 it('does not publish a discarded idea even if its old approval remains',async()=>{
  h.idea.status='descartada';expect(await runIdeaPost(database(),'idea','owner')).toHaveProperty('skipped');expect(h.post).not.toHaveBeenCalled()
 })
 it('blocks an expired automatic time on the current PR day',async()=>{
  h.idea.publish_date='2026-09-07';expect(await runIdeaPost(database(),'idea','owner')).toHaveProperty('skipped');expect(h.post).not.toHaveBeenCalled()
 })
})
