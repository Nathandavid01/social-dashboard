...
export type EntregaStageKey = (typeof ENTREGA_STAGES)[number]['key']
 ...
 */
export function computeStage(idea: StageInput): EntregaStageKey {
...
const STAGE_ORDER = ENTREGA_STAGES.map((s) => s.key)

/** Adjacent stage in the given direction, or null at the ends. */
export function adjacentStage(stage: EntregaStageKey, dir: 1 | -1): EntregaStageKey | null {
 ...
 */
export function stageToStatus(stage: EntregaStageKey): 'producida' | 'publicada' {
  switch (stage) {
    case 'edited':
    case 'approval':
...
export function bucketByStage<T extends StageInput>(ideas: T[]): Record<EntregaStageKey, T[]> {
  const out = Object.fromEntries(
    ENTREGA_STAGES.map((s) => [s.key, [] as T[]]),
  ) as Record<EntregaStageKey, T[]>
