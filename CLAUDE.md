...
- **Signed downloads (R2).** Every `ResponseContentDisposition` is `'inline'` or `attachmentDisposition(name)` (`lib/utils/content-disposition.ts`) — never interpolate `filename="${…}"`: R2 forwards the raw UTF-8 bytes and «La Güira» arrives as «La GÃ¼ira». Guarded by `lib/utils/content-disposition-guard.test.ts`. On the client, follow the signed URL with `followDownload(url)` (`lib/utils/follow-download.ts`); `window.open(url, '_blank')` after an `await` is blocked by Safari/iOS.
- **Posts made by hand in Metricool.** The Metricool API returns no file size, name or duration, but Metricool stores the uploaded file byte for byte — so a Recibo cut is linked to a hand-made post by exact `size_bytes` (`lib/recibo/match-published.ts`, run by `runReciboPublishedMatch` on opening Recibo and in the `metricool-sync` cron). Linking writes `metricool_post_id`/`posted_at` only where both are still null; drafts never link.
